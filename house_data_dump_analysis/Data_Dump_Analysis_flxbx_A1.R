...
setwd('/Users/Diego/Desktop/Data/Nicaragua/implementation_nicaragua/DUMPS/DUMP1')


############  Reading the data in - simply change the file name to analyze another data


ambient <- read.csv('flxbxA1.8.03.2015.sqld_ambient_table.csv')
inside <- read.csv('flxbxA1.8.03.2015.sqld_inside_table.csv')
refrigerator <- read.csv('flxbxA1.8.03.2015.sqld_mfi_table.csv')
switch <- read.csv('flxbxA1.8.03.2015.sqld_switch_table.csv')

#Note the data for the second check was missing from A1. What happened? I'm using the data from 
#the previous month just to check
house <- read.csv('flxbxA1.8.03.2015.sqld_zwave_table.csv')

#NOTE THIS WAS MISSING THE HOUSE DATA
...
############# Plotting an Entire Week - SUPER HARD - THERE IS A LOT OF DATA - 
...
plot(inside$time_stamp,inside$temp1, xlab='Day',ylab='Inside T1 (C)',cex=0.1,ylim=c(-10,30))
par(new=TRUE)
plot(inside$time_stamp,inside$temp2, xlab='Day',ylab='Inside T2 (C)',cex=0.1, col='red',ylim=c(-10,30))

plot(refrigerator$time_stamp,refrigerator$power,xlab='Day',ylab='Fridge (W)',cex=0.2,pch=19,ylim=c(0,1500))
lines(refrigerator$time_stamp, refrigerator$power)

plot(switch$time_stamp,switch$open,xlab='Day',ylab='Openings')

plot(house$time_stamp,house$house_Power,xlab='Day',ylab='House (W)',cex=0.1,ylim=c(0,1000))
...
plot(refrigerator$time_stamp,refrigerator$power,xlab='Day',ylab='Fridge (W)',cex=0.2,pch=19,ylim=c(0,400))
lines(refrigerator$time_stamp, refrigerator$power)
...
day_ambient <- subset(ambient,ambient$date == '2015-07-18')
day_inside <- subset(inside, inside$date == '2015-07-18')
day_refrigerator <- subset(refrigerator, refrigerator$date == '2015-07-18')
day_switch <- subset(switch, switch$date == '2015-07-18')
day_house <- subset(house, house$date == '2015-07-18')



par(mfrow=c(5,1),mai=c(0.8,0.8,0.3,0.3))
...
par(new=TRUE)
plot(day_inside$time_stamp, day_inside$temp2, xlab='Day',ylab='Inside T2 (C)',cex=0.1, col='red',ylim=c(0,30))

#NOTE: Adding a variable with NAs to complete the columns so we can plot the same time stamps
fridgepower_NAS <- c(day_refrigerator$power,rep(NA,length(day_ambient$time_stamp)-length(day_refrigerator$power)))
date_NAS <-  c(day_refrigerator$power,rep(NA,length(day_ambient$time_stamp)-length(day_refrigerator$power)))

plot(day_ambient$time_stamp, fridgepower_NAS ,xlab='Day',ylab='Fridge (W)',cex=0.2,pch=19,na.rm=TRUE)
lines(day_ambient$time_stamp, fridgepower_NAS)
...
plot(five_refrigerator$time_stamp, five_refrigerator$power,ylim=c(-10,400),xlab='Day',ylab='Fridge (W)',cex=0.2,pch=19)
...
plot(one_inside$time_stamp, one_inside$temp1, xlab='Day',ylab='Inside T1 (C)',cex=0.1, ylim=c(-10,40))
par(new=TRUE)
plot(one_inside$time_stamp, one_inside$temp2, xlab='Day',ylab='Inside T2 (C)',cex=0.1,ylim=c(-10,40), col='red')
...
plot(seven_inside$time_stamp, seven_inside$temp1, xlab='Day',ylab='Inside T1 (C)',cex=0.1, ylim=c(-10,40))
par(new=TRUE)
plot(seven_inside$time_stamp, seven_inside$temp2, xlab='Day',ylab='Inside T2 (C)',cex=0.1,ylim=c(-10,40), col='red')
plot(seven_refrigerator$time_stamp, seven_refrigerator$power,ylim=c(0,180),xlab='Day',ylab='Fridge (W)',cex=0.2,pch=19)
