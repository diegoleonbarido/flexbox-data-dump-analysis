...
e_i_group_tariff <- merge(e_i_group,group_tariff_code,by='Casa')
analysis_subset <- subset(e_i_group_tariff,e_i_group_tariff$Ano<2016 & e_i_group_tariff$Mes!="Noviembre"| e_i_group_tariff$Ano<2016 & e_i_group_tariff$Mes!="Diciembre")
...
for(i in 1:length(unique_months$Ano)){
  ...
  for(j in 1:length(subset_test$Casa)){
    ...
    importe <- if(group_analysis=='control'){subset_test$importe[j] * (subset_test$median.val[j])}else{subset_test$importe[j] * (subset_test$median.val[j])/100}
...
subset_test_append <- as.data.frame(subset_test_append)[c("Mes","Ano","Casa","energia")]

return(subset_test_append)
}
